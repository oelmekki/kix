(function() {
  ...
  exports.Board = new Class({
    Implements: Events,
    initialize: function(config, run) {
      if (run == null) run = true;
      this.players = {};
      this.config = config;
      this.step = config.step;
      this.run_step = config.run_step;
      if (run) setInterval(this.move.bind(this), config.base_beat);
      return this;
    },
    ...
    fixPosition: (function(new_position) {
      ...
      if (new_position.y > this.config.area_height) {
        return new_position.y = this.config.area_height;
      }
    }).protect(),
